                ...
                database()
                    ...
                    .then(() => {
                        database()
                            ...
                            .update(form)
                            .then(() => console.log("새로운 메뉴가 추가되었습니다 !"));
                    })
...
export default AddMenuModal = (props) => {
    ...
    if (isAddMenu) {
        ...
        const [inputDataCost, setInputDataCost] = useState([]);
            ...
            setSubMenu(subMenu.concat(
                <View style={{ flexDirection: 'row' }}>
                    <TextInput
                        ...
                        onChangeText={(text) => addValues(text, index, false)}
                    />
                    {
                        category === '음료'
                            ? <></>
                            : <TextInput
                                style={modalItem.addSubMenuTextInput}
                                placeholder='가격를 입력해주세요.'
                                onChangeText={(text) => addValues(text, index, true)}
        ...
        const removeTextInput = () => {
            ...
            if (category !== '음료')
                setInputDataCost(inputDataCost.filter((value, index) => index !== (subMenu.length - 1)));
        }

        //function to add text from TextInputs into single array
        const addValues = (text, index, isDessert) => {
            if (!isDessert) {
                ...
                if (check)
                    setInputDataCost(costArray);
                else {
                    costArray.push({ 'index': index, 'cost': text });
                    setInputDataCost(costArray);
                }
            }
        }
        ...
        const autoSetForm = () => {
            ...
            if (category === '음료') {
                ...
                if (inputData.length > 0) {
                    ...
                    console.log('> inputDataa : ' + JSON.stringify(inputData));
                    console.log('> inputDataaCost : ' + JSON.stringify(inputDataCost));


                    inputData.forEach((dataItem, dataIndex) => {

                        inputDataCost.forEach((costItem, costIndex) => {

                            if (dataIndex === costIndex) {
                                        ...
                                        ({ item }) => {
                                            ...
                                            return (
                                                <TouchableOpacity style={
                                                    ...
                                                    onPress={() => [setCategory(item), setSubMenu([]), setSubMenuAvailable(false)]}
                                                >
                                        ...
                                        category === '디저트' ?
                                            <>
                                                <View style={{ width: '100%', height: 150 }}>
                                                    <Text style={[modalItem.addMenuLeftText, { width: '100%', marginVertical: 5 }]}>카테고리 선택 및 추가</Text>
                                                    <FlatList
                                                        data={bakeryData.concat({ category_name: '+' })}
                                                        keyExtractor={item => item.toString()}
                                                        style={{ maxHeight: 140, height: 120 }}
                                                        numColumns={5}
                                                        scrollEnabled={false}
                                                ...
                                                <View style={modalItem.addMenu}>
                                                    ...
                                                    <TextInput
                                                        ...
                                                        onChangeText={(text) => setHotCost(text)}
                                            ...
                                            :
                                            <>
                                                <Text style={[modalItem.addMenuLeftText, { width: '100%', textAlign: 'center' }]}>위에서 종류를 선택해주세요</Text>
                                            </>
                                }
